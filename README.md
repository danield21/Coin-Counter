...
2. Using the location specified in `web/yaml/index.yml`, create another file at that location, relative to `web/yaml` and postfixed with `.yml`. So, if the location was `/language/en`, there should be a corresponding file at `web/yaml/language/en.yml`.
    ...
    3. Create another file which will contain the core translations of this application. If following the previous step, then the file should be `web/yaml/language/{language-code}/core.yml`. Follow `web/yaml/language/en/core.yml` as an example and for what to translate.
